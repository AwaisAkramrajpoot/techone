...
export function AdvancedView() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (row) =>
        ...
        row.createdDate.toLowerCase().includes(term)
    );
  }, [rows, search]);
  ...
  const handleFieldChange = <K extends keyof AdvancedFormValues>(
    field: K,
  ...
  const handleSave = () => {
    const nextErrors: Partial<Record<keyof AdvancedFormValues, string>> = {};
    ...
    const newRow: AdvancedRow = {
      id: nextId,
      code: `#${String(nextId).padStart(4, "0")}`,
      name: formValues.advanceName,
      createdDate: formatDateLabel(formValues.applyDate),
      type: parsedType,
      status: formValues.status ? "Active" : "Expired",
    };
