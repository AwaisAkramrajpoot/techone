...
import {
  allowanceDeductionDummyRows,
  type AllowanceDeductionRow,
  type AllowanceDeductionStatus,
  type AllowanceDeductionType,
} from "@/components/hr/data/allowance-deduction-data";
import {
...
export function AllowanceDeductionView() {
  const hrLayout = useHrLayout();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [rows, setRows] = useState<AllowanceDeductionRow[]>([]);
  const [search, setSearch] = useState("");
  const [errors, setErrors] = useState<
    Partial<Record<keyof AllowanceDeductionFormValues, string>>
  >({});
  const [formValues, setFormValues] = useState<AllowanceDeductionFormValues>(
    defaultFormValues
  );

  const statusClass = (status: AllowanceDeductionStatus) => {
  ...
  const typeClass = (type: AllowanceDeductionType) => {
    return type === "Allowance"
      ? "bg-[#DFF4FF] text-[#2EA8DF]"
      : "bg-[#FFD9EC] text-[#D63384]";
  };
  ...
  const handleFieldChange = <K extends keyof AllowanceDeductionFormValues>(
    field: K,
  ...
  const handleSave = () => {
    const nextErrors: Partial<Record<keyof AllowanceDeductionFormValues, string>> =
      {};
    if (!formValues.type) nextErrors.type = "Please fill this field";
    if (!formValues.name) nextErrors.name = "Please fill this field";
    if (!formValues.amount) nextErrors.amount = "Please fill this field";
    if (!formValues.code) nextErrors.code = "Please fill this field";
    ...
    const seed = rows.length === 0 ? allowanceDeductionDummyRows : [];
    const nextId = [...seed, ...rows].length + 1;
    const newRow: AllowanceDeductionRow = {
      id: nextId,
      type: formValues.type as AllowanceDeductionType,
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#667085]">
            Manage, configure, and track employee allowances and deductions
          </p>
        </div>
      ...
      {filteredRows.length === 0 ? (
        <div className="flex min-h-[360px] items-center justify-center">
          <div className="text-center">
            ...
            Add Allowances & Deductions First!
            </p>
          </div>
        </div>
      ) : (
            ...
            <table className="w-full min-w-[980px] border-collapse text-sm">
              <thead>
                <tr className="bg-[#F8FAFC] text-left text-[#111827]">
                  ...
                  <th className="px-3 py-3 font-semibold">Amount/percen...</th>
