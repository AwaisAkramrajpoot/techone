...
import {
  ...
  type GradeRow,
  type GradeStatus,
} from "@/components/hr/data/grade-data";
import { GradeModal, type GradeFormValues } from "@/components/hr/modals/GradeModal";

const defaultFormValues: GradeFormValues = {
  company: "",
  branch: "",
  department: "",
  designation: "",
...
export function GradeView() {
  ...
  const statusClass = (status: GradeStatus) => {
  ...
  const handleSave = () => {
    ...
    const seed = rows.length === 0 ? gradeDummyRows : [];
    ...
    const newRow: GradeRow = {
      ...
      gradeName: formValues.gradeName,
  ...
  return (
    <section className="min-h-[560px] flex-1 font-[poppins]">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[#0C4BA7] font-[inter]">Grade Management</h1>
          <p className="mt-1 text-sm text-[#667085]">
            Simplify employee grading and pay structures
          </p>
        </div>
            ...
            <table className="w-full min-w-[820px] border-collapse text-sm">
              <thead>
                <tr className="bg-[#F8FAFC] text-left text-[#111827]">
                  ...
                  <th className="px-3 py-3 font-semibold">Grade Name</th>
                  <th className="px-3 py-3 font-semibold">No Of Employees</th>
              ...
              <tbody>
                {filteredRows.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-[#374151]">{row.gradeName}</td>
                    <td className="px-3 py-3 text-[#374151]">{row.employees}</td>
