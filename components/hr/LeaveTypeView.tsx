...
} from "@/components/hr/data/leave-type-data";
import {
  LeaveTypeModal,
  type LeaveTypeFormValues,
} from "@/components/hr/modals/LeaveTypeModal";

const defaultFormValues: LeaveTypeFormValues = {
  company: "",
  branch: "",
  leaveType: "",
  abbreviation: "",
...
export function LeaveTypeView() {
  ...
  const statusClass = (status: LeaveTypeStatus) => {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (row) =>
        ...
        row.abbreviation.toLowerCase().includes(term) ||
        row.maxDays.toLowerCase().includes(term)
    );
  }, [rows, search]);
  ...
  const handleSave = () => {
    ...
    const seed = rows.length === 0 ? leaveTypeDummyRows : [];
    const nextId = [...seed, ...rows].length + 1;

    const newRow: LeaveTypeRow = {
      id: nextId,
      leaveType: formValues.leaveType,
      subtitle: formValues.description || "Paid time off for vacation",
      abbreviation: formValues.abbreviation,
      maxDays: `${formValues.maxDays} days`,
      paid: true,
      status: "Active",
    };
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#667085]">
            Organize leave types for employee time-off management
          </p>
        </div>
        <div className="flex flex-nowrap gap-1">
          <Button
            className="h-8 px-2.5 text-xs shrink-0 bg-[#04499E] hover:bg-[#033E87] text-white text-[13px]"
          ...
          <Button
            className="h-8 px-2.5 text-xs shrink-0 bg-[#04499E] hover:bg-[#033E87] text-white text-[13px] gap-1"
          >
            <Image src="/svgs/report.svg" alt="" width={18} height={18} />
            Report
          </Button>
          <Button
            className="bg-[#04499E] h-8 px-2.5 hover:bg-[#033E87] text-[13px] text-white gap-1"
          >
            ...
            <table className="w-full min-w-[860px] border-collapse text-sm">
              <thead>
                <tr className="bg-[#F8FAFC] text-left text-[#111827]">
                  ...
                  <th className="px-3 py-3 font-semibold">Abbreviation</th>
                  <th className="px-3 py-3 font-semibold">Max Days</th>
                ...
                {filteredRows.map((row, index) => (
                  ...
                  >
                    <td className="px-3 py-3 text-[#374151]">
                      ...
                      <p className="text-xs text-[#98A2B3]">{row.subtitle}</p>
                    </td>
                    <td className="px-3 py-3 text-[#374151]">{row.abbreviation}</td>
                    <td className="px-3 py-3 text-[#374151]">{row.maxDays}</td>
                    <td className="px-3 py-3 text-[#0B63CE]">
                      {row.paid ? <Check className="h-4 w-4" /> : null}
                    </td>
